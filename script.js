...
window.addEventListener("scroll", handleHeaderScroll);
  ...
  document.getElementById("registrationForm").addEventListener("submit", function(e) {
    ...
    if (!mobile) {
        ...
        errors.mobileError = "Enter valid 10-digit mobile number.";
        isValid = false;
    }

    // Alternate mobile validation
    if (!altMobile) {
        errors.altMobileError = "Alternate mobile is required.";
        isValid = false;
    } else if (!phonePattern.test(altMobile)) {
        errors.altMobileError = "Enter valid 10-digit alternate mobile.";
        isValid = false;
    }
    ...
    // Program validation
    if (!program) {
        errors.programError = "Please select program.";
        isValid = false;
    }
...
document.querySelectorAll(".hamb_drop > a").forEach(menu => {
    menu.addEventListener("click", function(e) {
        e.preventDefault();

        let parent = this.parentElement;
        let submenu = parent.querySelector(".hamb_sub");
        let icon = this.querySelector("span, i, .plus-icon");
